...
Renderer::Renderer(const std::string &name, unsigned int width, unsigned int height, Map &map) : map_(map), running_(true), last_update(0) {
    if (name.empty()) {
        window   = nullptr;
        renderer = nullptr;
...
void Renderer::show(float fps) {
    ...
    // Clear the renderer and copy the results
